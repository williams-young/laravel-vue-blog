<template>
    <div id="root" :class="$style.wrap" :style="{height:wrapHeight+'px'}">
        <AlertWithLoading v-show="doShowLoading"/>
        <AlertWithText v-show="alertText !== ''" :text="alertText" :onclick="() => {$store.commit(HIDE_ALERTTEXT)}"/>
...
<script>
    ...
    import { ADMIN_URL } from '@/components/API'
    ...
    export default {
        ...
        mounted() {
            window.addEventListener('deicemotion', throttle(this.testShake))
            this.setWrapHeight()
        },
        beforeDestroy() {
